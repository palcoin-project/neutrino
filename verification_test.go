...
func TestVerifyBlockFilter(t *testing.T) {
	...
	b := palcutil.NewBlock(spendBlock)
...
func spendNP2WKH(t *testing.T, pubKey *btcec.PublicKey, prevTx *wire.MsgTx,
	...
	pkHash := palcutil.Hash160(pubKey.SerializeCompressed())

	witAddr, err := palcutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2PK(t *testing.T, pubKey *btcec.PublicKey) []byte {
	addr, err := palcutil.NewAddressPubKey(
...
func makeP2PKH(t *testing.T, pubKey *btcec.PublicKey) []byte {
	pkHash := palcutil.Hash160(pubKey.SerializeCompressed())
	addr, err := palcutil.NewAddressPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2SH(t *testing.T, pubKey *btcec.PublicKey) []byte {
	...
	addr, err := palcutil.NewAddressScriptHash(script, chainParams)
	require.NoError(t, err)
...
func makeNP2WKH(t *testing.T, pubKey *btcec.PublicKey) []byte {
	...
	addr, err := palcutil.NewAddressScriptHash(witnessProgram, chainParams)
	require.NoError(t, err)
...
func makeP2WKH(t *testing.T, pubKey *btcec.PublicKey) []byte {
	...
	addr, err := palcutil.NewAddressWitnessPubKeyHash(pkHash, chainParams)
	require.NoError(t, err)
...
func makeP2WSH(t *testing.T, pubKey *btcec.PublicKey) []byte {
	...
	addr, err := palcutil.NewAddressWitnessScriptHash(
