...
var (
	...
	ourKnownTxsByBlock = make(map[chainhash.Hash][]*palcutil.Tx)

	// ourKnownTxsByFilteredBlock lets the rescan goroutine keep track of
	// transactions we're interested in that are in the blockchain we're
	// following as signalled by OnFilteredBlockConnected and
	// OnFilteredBlockDisconnected.
	ourKnownTxsByFilteredBlock = make(map[chainhash.Hash][]*palcutil.Tx)
)

// secSource is an implementation of palcwallet/txauthor/SecretsSource that
...
func (s *secSource) add(privKey *btcec.PrivateKey) (palcutil.Address, error) {
	pubKeyHash := palcutil.Hash160(privKey.PubKey().SerializeCompressed())
	addr, err := palcutil.NewAddressWitnessPubKeyHash(pubKeyHash, s.params)
	if err != nil {
		return nil, err
	}
...
func (s *secSource) GetKey(addr palcutil.Address) (*btcec.PrivateKey, bool,
	error) {
...
func (s *secSource) GetScript(addr palcutil.Address) ([]byte, error) {
...
var (
	...
	secSrc                    *secSource
	addr1, addr2, addr3       palcutil.Address
...
func testRescan(harness *neutrinoHarness, t *testing.T) {
	...
	newPkScript := func() (palcutil.Address, []byte, *wire.TxOut) {
		t.Helper()
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	inSrc := func(tx wire.MsgTx) func(target palcutil.Amount) (
		total palcutil.Amount, inputs []*wire.TxIn,
		inputValues []palcutil.Amount, scripts [][]byte, err error) {
		...
		return func(target palcutil.Amount) (total palcutil.Amount,
			inputs []*wire.TxIn, inputValues []palcutil.Amount,
			...
			inputValues = []palcutil.Amount{
				palcutil.Amount(tx.TxOut[ourIndex].Value)}
	...
	_, err = harness.h1.GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
		[]*palcutil.Tx{}, rpctest.BlockVersion, time.Time{},
		[]wire.TxOut{{
			Value:    0,
			PkScript: []byte{},
		}})
...
func startRescan(t *testing.T, svc *neutrino.ChainService, addr palcutil.Address,
	...
	rescan := neutrino.NewRescan(
		...
		neutrino.NotificationHandlers(
			rpcclient.NotificationHandlers{
				...
				OnRedeemingTx: func(tx *palcutil.Tx,
					details *btcjson.BlockDetails) {
					rescanMtx.Lock()
					hash, err := chainhash.
						NewHashFromStr(
